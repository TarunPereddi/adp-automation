...
export async function ensureIndexes(db: Db): Promise<void> {
  await Promise.all([
    ...
    db.collection('portal_sessions').createIndex({ accountId: 1 }, { unique: true }),
    db.collection('portal_sessions').createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 }),
    ...
    db.collection('calendar_checks').createIndex({ accountId: 1, dateKey: 1 }, { unique: true }),
    db.collection('calendar_checks').createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 }),
